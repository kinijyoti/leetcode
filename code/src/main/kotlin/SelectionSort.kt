/*
The selection sort algorithm is similar to how humans often sort things. For example, if you're organizing a hand of cards, you can find the lowest value card and put it in the left-most spot. Then, find the lowest of the remaining cards and place it to the right of the first card. Repeat until all the cards are in order.
...
Initialize: Start by assuming the first element is the smallest.
Search: Compare each remaining unsorted element to the current minimum. If a smaller element is found, update the current minimum.
Swap: After finding the smallest element, swap it with the first unsorted element.
